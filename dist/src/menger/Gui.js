...
export class GUI {
    ...
    onKeydown(key) {
        ...
        switch (key.code) {
            case "KeyW": {
                console.log("w");
                let temp = this.camera.pos().add(this.camera.forward().scale(-GUI.zoomSpeed));
                this.camera.setPos(temp);
                break;
            }
            case "KeyA": {
                let temp = this.camera.pos().add(this.camera.right().scale(GUI.panSpeed));
                this.camera.setPos(temp);
                break;
            }
            case "KeyS": {
            ...
            case "ArrowLeft": {
                // let temp: Vec3 = this.camera.pos().add(this.camera.rotate())
